...
| `C-c m p` | Live browser preview (impatient-showdown) |
| `C-c m g` | GitHub-style preview via grip (Linux) |
| `C-c m w` | Terminal preview with glow |
...
# then: C-c m f  (focus)   C-c m m  (toggle markup)   C-c m p  (browser preview)
```
